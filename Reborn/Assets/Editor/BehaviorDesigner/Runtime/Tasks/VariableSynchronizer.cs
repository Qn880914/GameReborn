...
namespace FrameWork.BehaviorDesigner.Runtime
{
    ...
    public class VariableSynchronizer : MonoBehaviour
    {
        ...
        [SerializeField]
        private List<VariableSynchronizer.SynchronizedVariable> m_SynchronizedVariables = new List<VariableSynchronizer.SynchronizedVariable>();
        public List<VariableSynchronizer.SynchronizedVariable> synchronizedVariables
        {
            get
            {
                return this.m_SynchronizedVariables;
            }
            set
            {
                this.m_SynchronizedVariables = value;
                base.enabled = true;
            }
        }
        ...
        private void UpdateIntervalChanged()
        {
            ...
            if (this.m_UpdateInterval == UpdateIntervalType.EveryFrame)
            {
                base.enabled = true;
            }
            else if (this.m_UpdateInterval == UpdateIntervalType.SpecifySeconds)
            {
                if (Application.isPlaying)
                {
                    this.m_UpdateWait = new WaitForSeconds(this.m_UpdateIntervalSeconds);
        ...
        public void Awake()
        {
            for (int i = this.m_SynchronizedVariables.Count - 1; i > -1; i--)
            {
                VariableSynchronizer.SynchronizedVariable synchronizedVariable = this.m_SynchronizedVariables[i];
        ...
        public void Tick()
        {
            for (int i = 0; i < this.m_SynchronizedVariables.Count; i++)
            {
                VariableSynchronizer.SynchronizedVariable synchronizedVariable = this.m_SynchronizedVariables[i];
                switch (synchronizedVariable.synchronizationType)
                {
